<template>
  <div class="server">
   <div class="tabBox">
     <div class="tabLi"  v-for="(item, index) in tabs" :key="index" @click ="showTabCon(index,item.code)">
...
export default {
  ...
  methods: {
    ...
    editHosiTr (option) {
      ...
      this.ifNewHosAdd = false
      this.hosForm.id = option.id
      this.hosForm.hospitalName = option.hospitalName
      this.hosForm.type = option.hospitalType.split(',')
      this.ishos = true
      this.issetN = false
      this.dialogTitle2 = '编辑医院信息'
    },
    // 添加医院
    addHospital () {
      this.ifNewHosAdd = true
      this.dialogFormVisible2 = true
      this.ishos = true
      this.issetN = false
      this.dialogTitle2 = '添加医院'
    },
    // 保存 医院修改新增数据
    saveHos () {
      let self = this
